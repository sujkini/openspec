...
echo ""
echo "CI Monitor (local):"
echo "  After /opsx-apply, run /opsx-ci-monitor <change-name> to analyze PR CI"
echo "  CLI: bash openspec/ci-monitor/scripts/run-local.sh --pr-url <URL> --change <name>"
echo "  Configure: openspec/config.yaml → ci_monitor (runtime: local | prow | both)"
echo "  Optional Prow: openspec/ci-monitor/docs/prow-ci-operator-config.yaml"
echo ""
echo "Feedback:"
