...
1. Commit generated test files to the PR branch:
   ```bash
   # In fork working copy
...
2. ASK: "Run E2E tests now? (yes / no)"
   - **Yes** → execute the test suite:
     ```bash
     make test-e2e
     ```
     Report results (pass/fail per journey, failures with diagnostics).
   - **No** → skip execution; present summary of what was generated and pushed.
